...
您需在开发设备端时，使设备支持OTA升级。准备升级包文件时，可在原固件配置基础上进行配置更新，并编译为物联网平台支持的文件格式。本文提供设备端示例代码。

已在物联网平台控制台创建产品和设备，并获得设备证书信息。

本示例中，设备端开发使用阿里云提供的C语言Link SDK 3.x。请访问[Link SDK](https://www.alibabacloud.com/help/product/93051.htm)下载SDK Demo。

在SDK Demo中，fota即OTA升级。

## 设备接入平台

1.  使用make menuconfig命令进入编译配置。在编译选项中，选择FEATURE\_OTA\_ENABLED \(NEW\)，开启设备端支持OTA升级。

    ![支持OTA升级](https://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/1731649951/p66021.png)

2.  进入src/dev\_model/examples目录下的linkkit\_example\_solo.c示例代码。

    该示例文件中，以下代码就是OTA升级代码。
...
3.  在linkkit\_example\_solo.c文件中，配置设备证书信息。
...
1.  根据业务需要，在linkkit\_example\_solo.c中更新配置。

    本示例中，更新后的文件请参见本文附录章节。

2.  编译更新后的linkkit\_example\_solo.c文件，并保存编译后的.bin文件。

    本示例中，将该.bin文件保存名为app\_1001。

    在下一步[推送固件到设备端](/intl.zh-CN/最佳实践/监控运维/设备OTA固件升级实践/推送固件到设备端.md)，您需在物联网平台控制台添加升级包，上传该.bin文件。
